...
//    if (dbus_message_get_path(message) != NULL && g_ascii_strcasecmp(dbus_message_get_path(message),path) == 0) {
	...
	if (message_type == DBUS_MESSAGE_TYPE_SIGNAL) {
		...
		(dbus_message_get_member(message), "Ready") == 0) {
		dbus_error_init(&error);
		if (dbus_message_get_args
		    (message, &error, DBUS_TYPE_INT32, &i,
		     DBUS_TYPE_INVALID)) {
		...
		instance->cache_size =
		    request_int_value(instance, item, "GetCacheSize");
		//printf("cache size = %i\n",instance->cache_size);
		return DBUS_HANDLER_RESULT_HANDLED;
	    }
	    if (g_ascii_strcasecmp
		(dbus_message_get_member(message), "Cancel") == 0) {
		dbus_error_init(&error);
		if (dbus_message_get_args
		    (message, &error, DBUS_TYPE_INT32, &i,
		     DBUS_TYPE_INVALID)) {
		    list_mark_controlid_cancelled(instance->playlist, i,
						  TRUE);
		} else {
		    dbus_error_free(&error);
		}
	    ...
	    if (g_ascii_strcasecmp
		(dbus_message_get_member(message), "RequestById") == 0) {
		dbus_error_init(&error);
		if (dbus_message_get_args
		    ...
		    printf("Got id %s\n", s);
		    item =
			list_find_by_id(instance->playlist,
					(gint) g_strtod(s, NULL));
		    if (item != NULL) {
			item->play = TRUE;
			...
			printf("id %s has newwindow = %i\n", s,
			       item->newwindow);
			if (item->newwindow == 0) {
			    send_signal_with_boolean(instance, item,
						     "SetShowControls",
						     TRUE);
			    if (item->streaming) {
				send_signal_with_string(instance, item,
							"Open", item->src);
			    } else {
				NPN_GetURLNotify(instance->mInstance,
						 item->src, NULL, item);
			    }
			} else {
			    i = 0;
			    ...
			    rand = g_rand_new();
			    item->controlid =
				g_rand_int_range(rand, 0, 65535);
			    g_rand_free(rand);
			    tmp =
				g_strdup_printf("/control/%i",
						item->controlid);
	    ...
	    if (g_ascii_strcasecmp
		(dbus_message_get_member(message), "Next") == 0) {
		...
		if (instance->lastopened != NULL
			...
			if (instance->lastopened->loopcount < 0) {
			    ...
			    instance->lastopened->loop = FALSE;
			    item =
				list_find_next_playable(instance->
							playlist);
			}
		    }
		}
...
DBusConnection *dbus_hookup(nsPluginInstance * instance)
{
	...
	instance->dbus_dispatch =
	    g_thread_create(dbus_dispatcher, instance, TRUE, NULL);
    } else {
	dbus_connection_setup_with_g_main(connection, NULL);
    }

    dbus_bus_add_match(connection,
		       "type='signal',interface='com.gecko.mediaplayer'",
		       NULL);
...
DBusConnection *dbus_unhook(DBusConnection * connection,
			    nsPluginInstance * instance)
{
...
void open_location(nsPluginInstance * instance, ListItem * item,
		   gboolean uselocal)
{
    DBusMessage *message;
    const char *file;
	...
	if (!item->opened) {
	    ...
	    argvn[arg++] =
		g_strdup_printf("--controlid=%i", instance->controlid);
	...
	if (item->hrefid == 0) {
	    ...
	    message =
		dbus_message_new_signal(path, "com.gnome.mplayer",
					"OpenButton");
	    dbus_message_append_args(message, DBUS_TYPE_STRING, &file,
				     DBUS_TYPE_STRING, &id,
				     DBUS_TYPE_INVALID);
...
void resize_window(nsPluginInstance * instance, ListItem * item, gint x,
		   gint y)
{
    DBusMessage *message;
    gchar *path;
...
void send_signal(nsPluginInstance * instance, ListItem * item,
		 gchar * signal)
{
    DBusMessage *message;
...
void send_signal_when_ready(nsPluginInstance * instance, ListItem * item,
			    gchar * signal)
{
    DBusMessage *message;
    const char *localsignal;
			     ...
			     gchar * signal, gchar * str)
{
	...
	message =
	    dbus_message_new_signal(path, "com.gnome.mplayer",
				    localsignal);
	dbus_message_append_args(message, DBUS_TYPE_STRING, &localstr,
				 DBUS_TYPE_INVALID);
	dbus_connection_send(instance->connection, message, NULL);
	dbus_message_unref(message);
    }
			     ...
			     gchar * signal, gdouble dbl)
{
	...
	message =
	    dbus_message_new_signal(path, "com.gnome.mplayer",
				    localsignal);
	dbus_message_append_args(message, DBUS_TYPE_DOUBLE, &dbl,
				 DBUS_TYPE_INVALID);
	dbus_connection_send(instance->connection, message, NULL);
	dbus_message_unref(message);
    }
			      ...
			      gchar * signal, gboolean boolean)
{
	...
	message =
	    dbus_message_new_signal(path, "com.gnome.mplayer",
				    localsignal);
	dbus_message_append_args(message, DBUS_TYPE_BOOLEAN, &boolean,
				 DBUS_TYPE_INVALID);
	dbus_connection_send(instance->connection, message, NULL);
	dbus_message_unref(message);
    }
...
gboolean request_boolean_value(nsPluginInstance * instance,
			       ListItem * item, gchar * member)
{
    DBusMessage *message;
	...
	message =
	    dbus_message_new_method_call(dest, path, "com.gnome.mplayer",
					 localmember);
	dbus_error_init(&error);
	replymessage =
	    dbus_connection_send_with_reply_and_block(instance->connection,
						      message, -1, &error);
	if (dbus_error_is_set(&error)) {
	    printf("Error message = %s\n", error.message);
	}
	dbus_message_get_args(replymessage, &error, DBUS_TYPE_BOOLEAN,
...
gdouble request_double_value(nsPluginInstance * instance, ListItem * item,
			     gchar * member)
{
    DBusMessage *message;
    DBusMessage *replymessage;
	...
	dbus_message_get_args(replymessage, &error, DBUS_TYPE_DOUBLE,
			      &result, DBUS_TYPE_INVALID);
	dbus_message_unref(message);
	dbus_message_unref(replymessage);
    }
...
gint request_int_value(nsPluginInstance * instance, ListItem * item,
		       gchar * member)
{
    DBusMessage *message;
    DBusMessage *replymessage;
	...
	dbus_message_get_args(replymessage, &error, DBUS_TYPE_INT32,
			      &result, DBUS_TYPE_INVALID);
	dbus_message_unref(message);
	dbus_message_unref(replymessage);
    }
...
gboolean is_valid_path(nsPluginInstance * instance, const char *message)
{
	...
	if (instance->playlist != NULL) {
	    for (iter = instance->playlist; iter != NULL;
...
gpointer dbus_dispatcher(gpointer data)
{
    ...
    while (instance != NULL
	   ...
	   && dbus_connection_read_write_dispatch(instance->connection,
						  10)) {
	// printf(".");
    }
